...
```{r}
v2 <- read.csv("Clean_Data/armed_conflict.csv", header =TRUE)

```

```{r}
names(v2)[names(v2) == "Mortality.x"] <- "Under_5_Mortality"
names(v2)[names(v2) == "Mortality.y"] <- "Infant_Mortality"
names(v2)[names(v2) == "Mortality.x.x"] <- "Neonatal_Mortality"
names(v2)[names(v2) == "Mortality.y.y"] <- "Maternal_Mortality"
...
```
```{r}
write.csv(v2, file = "Clean_Data/finaldata.csv", row.names = FALSE)

```

```{r}

finaldata <- read.csv("Clean_Data/finaldata.csv", header =TRUE)
